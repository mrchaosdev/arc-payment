...
another Arc-testnet project, against SealPay. Sources: the live site, the repository README,
...
## What Keryx has that SealPay does not

**Server tier and durable storage.** SealPay is entirely client-side: `src/lib/payments.ts` is
...
SealPay calls the USDC `transfer()` function directly, one transaction per payment.
...
in SealPay's README only as future milestones.
...
rather than the database before anything settles. SealPay deploys no contract.
...
SealPay stops at RainbowKit plus a per-transaction signature.
...
embeddable creator badge SVG. SealPay has five web routes.
...
swap atomically, health-gate, and roll back on failure. SealPay has one 27-line unit test file
...
- Art direction. Screenshots taken on 2026-09-09 showed Keryx committed to a single look — flat
  ...
  rule and a dispatch ticker, all matching its herald/toll/ledger vocabulary — while SealPay was
  ...
  SealPay has since been rebuilt on that same language — see
