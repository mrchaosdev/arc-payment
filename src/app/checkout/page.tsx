...
export default async function CheckoutPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="min-h-dvh"><TopBar workspace /><main className="px-4 py-8 sm:px-8 sm:py-12"><PaymentStudio key={JSON.stringify(request)} checkout initialMode="pay" initialRequest={request} /></main></div>;
}
