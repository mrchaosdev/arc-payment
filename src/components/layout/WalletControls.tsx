...
export function WalletControls({ address, chainId, openAccountModal }: {
  ...
  const balanceLabel = balance.isError ? t("balanceUnavailable") : exactBalance === undefined ? t("loadingUsdc") : `${exactBalance} USDC`;
  const isArc = chainId === ARC_CHAIN_ID;
  const networkLabel = chainId === 5_042 ? "Arc" : chainId === 5_042_002 ? "Arc Testnet" : t("networkUnknown");
  const balanceNote = chainId === 5_042 ? t("balanceNoteMainnet") : t("balanceNoteTestnet");

  async function switchToArc() {
    try { await switchChainAsync({ chainId: ARC_CHAIN_ID }); }
    catch { toast({ title: t("switchFailedTitle"), description: t("switchFailedBody"), tone: "error" }); }
  }
  async function copyAddress() {
    try { await navigator.clipboard.writeText(address); toast({ title: t("addressCopied"), tone: "success" }); }
    catch { toast({ title: t("copyUnavailableTitle"), description: t("copyUnavailableBody"), tone: "info" }); }
  }
  ...
  return <div className="top-bar-wallet-controls flex w-full min-w-0 items-center justify-between gap-2 sm:w-auto sm:justify-start">
    {isArc ? <span className="top-bar-network-status shrink-0 border border-[var(--border)] px-2 py-2 text-[11px] text-[var(--text-muted)]">{networkLabel}</span>
      : <button type="button" className="top-bar-switch-button shrink-0 border border-[var(--negative)] px-2 py-2 text-[11px] text-[var(--negative)] disabled:opacity-50" disabled={isPending} onClick={switchToArc}>
        {isPending ? t("switching") : t("switchToArc")}
      </button>}
    <button type="button" popoverTarget={walletId} aria-label={t("walletDetails")} title={balanceLabel}
    ...
    <div popover="auto" id={walletId} ref={walletPanel} aria-label={t("walletDetails")} className="top-bar-wallet-popover fixed left-auto right-4 top-28 m-0 w-80 max-w-[calc(100vw-2rem)] border border-[var(--border-strong)] bg-[var(--surface)] p-4 text-[var(--text-primary)] shadow-xl sm:top-16">
      <h2 className="top-bar-wallet-title text-sm font-semibold">{t("yourArcWallet")}</h2>
      ...
      <div className="top-bar-wallet-actions mt-4 flex flex-wrap gap-3 text-xs">
        <button className="top-bar-copy-address flex items-center gap-1 text-[var(--action)]" type="button" onClick={copyAddress}><Copy className="top-bar-copy-icon size-3" />{t("copyAddress")}</button>
        ...
        <button className="top-bar-refresh-balance flex items-center gap-1 text-[var(--text-muted)] disabled:opacity-50" type="button" disabled={balance.isFetching} onClick={() => void balance.refetch()}><RefreshCw className="top-bar-refresh-icon size-3" />{t("refreshBalance")}</button>
      </div>
      <button className="top-bar-manage-wallet mt-4 w-full border border-[var(--border)] p-2 text-xs" type="button" onClick={() => { walletPanel.current?.hidePopover(); openAccountModal(); }}>{t("manageWallet")}</button>
    </div>

    <div popover="auto" id={pendingId} ref={pendingPanel} aria-label={t("pendingPayments")} className="top-bar-pending-popover fixed left-auto right-4 top-28 m-0 max-h-[65dvh] w-80 max-w-[calc(100vw-2rem)] overflow-y-auto border border-[var(--border-strong)] bg-[var(--surface)] p-4 text-[var(--text-primary)] shadow-xl sm:top-16">
      <h2 className="top-bar-pending-title text-sm font-semibold">{t("pendingPayments")}</h2>
      <p className="top-bar-pending-description mt-2 text-xs text-[var(--text-muted)]">{t("pendingDescription")}</p>
      {!pending.length && <p role="status" className="top-bar-pending-empty mt-4 text-sm">{t("pendingEmpty")}</p>}
      <ul className="top-bar-pending-list mt-3 divide-y divide-[var(--border)]">
        {pending.map(payment => <li className="top-bar-pending-item py-3" key={payment.hash}>
          ...
          <a className="top-bar-pending-explorer mt-2 inline-block text-xs text-[var(--action)]" href={arcTransactionUrl(payment.hash)} target="_blank" rel="noreferrer">{t("awaitingReceipt")}</a>
        </li>)}
      </ul>
      <Link className="top-bar-activity-link mt-3 block text-xs text-[var(--action)]" href="/history" onClick={() => pendingPanel.current?.hidePopover()}>{t("viewAllActivity")}</Link>
    </div>
  </div>;
}
