...
export function PaymentStudio({ initialMode, initialRequest, checkout = false }: {
  ...
  async function prepare() {
    ...
    try {
      ...
      if (!account.address) throw new Error(t("connectToReview"));
  ...
  async function send() {
    ...
    try {
      ...
      if (submitted) {
        setStage("pending");
        setError(t("slowConfirmation"));
      } else { setStage("review"); setError(friendlyError(e, t)); }
  ...
  const summary = <>
    <div className="payment-studio-summary-header mb-7 flex items-start justify-between gap-3">
      <div className="payment-studio-summary-heading"><p className="payment-studio-summary-title text-lg font-semibold tracking-tight">{t("paymentRequest")}</p>
        <p className="payment-studio-summary-description mt-1.5 text-xs text-[var(--text-muted)]">{t("summaryHint")}</p></div>
      <Chip className="payment-studio-summary-chip" tone="muted">{t("badgeRequest")}</Chip>
    </div>
    <div className="payment-studio-summary-amount-block border border-[var(--border)] bg-[var(--surface-soft)] px-4 py-7 text-center">
      <Label className="payment-studio-summary-amount-label">{t("amountRequested")}</Label>
  ...
  const fields = <>
    <div className="payment-studio-fields-header mb-7 flex items-start justify-between gap-3">
      <div className="payment-studio-fields-heading"><p className="payment-studio-fields-title text-lg font-semibold tracking-tight">{mode === "pay" ? t("sendHeading") : t("requestHeading")}</p>
        <p className="payment-studio-fields-description mt-1.5 text-xs text-[var(--text-muted)]">{mode === "pay" ? t("sendSubheading") : t("requestSubheading")}</p></div>
      <Chip className="payment-studio-mode-chip" tone="muted">{mode === "pay" ? t("badgeTransfer") : t("badgeRequest")}</Chip>
    </div>
    <fieldset disabled={busy} className="payment-studio-fields space-y-5 disabled:opacity-70">
      {mode === "pay" && !checkout && hydrated && contacts.length > 0 && <label className="payment-studio-contact-label block text-xs text-[var(--text-muted)]">Saved recipient
        <select aria-label={t("savedRecipient")} className="payment-studio-contact-select payment-input mt-2" value="" onChange={event => { if (event.target.value) { edit("to", event.target.value); setFieldErrors(errors => ({ ...errors, to: undefined })); } }}>
          <option className="payment-studio-contact-placeholder" value="">{t("chooseContact")}</option>
      ...
      <Field label={mode === "pay" ? t("recipientAddress") : t("receiveTo")} error={fieldErrors.to} errorId="recipient-error"
        hint={mode === "request" ? t("receiveToHint") : undefined}>
        <input ref={toInput} aria-label={mode === "pay" ? t("recipientAddress") : t("receiveTo")}
      ...
      <Field label={t("amount")} error={fieldErrors.amount} errorId="amount-error"
        hint={isConnected
          ? t("amountHintConnected", {
              balance: balanceQuery.isError
                ? t("balanceUnavailable")
                : balanceQuery.data === undefined
                  ? t("balanceLoading")
                  : `${formatUnits(balanceQuery.data, 6)} USDC`,
            })
          : t("amountHintDisconnected")}>
        <div className={`payment-studio-amount-field flex items-center gap-3 border bg-[var(--surface)] px-4 transition-colors focus-within:border-[var(--action)] ${fieldErrors.amount ? "border-[var(--negative)]" : "border-[var(--border)]"}`}>
          <input ref={amountInput} aria-label={t("amount")} aria-invalid={fieldErrors.amount ? true : undefined}
      ...
      <div className="payment-studio-details-section border-t border-[var(--border)] pt-4">
        <button type="button" onClick={() => setShowDetails(open => !open)} aria-expanded={showDetails} aria-controls="payment-details"
          ...
          <span className="payment-studio-details-toggle-label">{showDetails ? t("hideDetails") : t("addDetails")}</span>
        ...
        <div id="payment-details" hidden={!showDetails} className="payment-studio-details-fields mt-5 space-y-5">
          <Field label={t("whatIsThisFor")} hint={t("memoHint")}>
            <input aria-label="Memo" className="payment-studio-memo-input payment-input" value={current.memo} onChange={e => edit("memo", e.target.value)} maxLength={120} placeholder={t("memoPlaceholder")} />
          </Field>
          <Field label={t("reference")} hint={t("optional")}>
            <input aria-label={t("reference")} className="payment-studio-reference-input payment-input" value={current.reference} onChange={e => edit("reference", e.target.value)} maxLength={48} placeholder={t("referencePlaceholder")} />
          </Field>
        </div>
      </div>
    </fieldset>
  ...
  const receipt = <>
    <div className="payment-studio-receipt-header mb-6 flex items-start justify-between gap-3">
      <div className="payment-studio-receipt-heading">
        <h2 className="payment-studio-receipt-title text-xl font-semibold tracking-tight">{stage === "review" ? t("reviewTitle") : stage === "signing" ? t("confirmInWallet") : stage === "success" ? t("paymentComplete") : stage === "failed" ? t("paymentNotCompleted") : t("waitingConfirmation")}</h2>
      ...
      <Chip className="payment-studio-receipt-status" tone={stage === "success" ? "positive" : stage === "failed" ? "negative" : "primary"}>
        {stage === "success" ? <><CheckCircle2 size={11} /> Settled</> : stage === "failed" ? t("stageReverted")
          : <><StatusDot /> {stage === "review" ? t("stageUnsigned") : stage === "signing" ? t("stageInWallet") : t("stageBroadcast")}</>}
      </Chip>
    </div>
    ...
    {stage === "pending" || stage === "signing" || stage === "success" ? <div className="payment-studio-progress mt-6">
      <ProgressBar label={stage === "success" ? t("confirmedLabel") : stage === "signing" ? t("signingLabel") : t("submittedLabel")} indeterminate={stage !== "success"} />
    ...
    {stage === "review" && <p className="payment-studio-review-notice mt-5 text-xs leading-5 text-[var(--text-muted)]">{t("reviewHint")}</p>}
  </>;
  ...
  const form = <div className="payment-studio-form p-5 sm:p-7">
    ...
    <div className="payment-studio-actions mt-6 space-y-3">
      {mode === "request" ? <Button type="button" onClick={createRequest} disabled={!!shareUrl} className="payment-studio-create-request-button h-12 w-full"><Link2 size={16} />{shareUrl ? t("requestCreated") : t("createLink")}</Button>
        ...
        : stage === "success" || stage === "failed" ? <>
            <Button type="button" variant="secondary" onClick={startNewPayment} className="payment-studio-new-payment-button w-full">{t("newPayment")}</Button>
            <Button type="button" variant="ghost" onClick={sendAgain} className="payment-studio-send-again-button w-full"><RotateCcw size={14} />{stage === "failed" ? t("tryAgain") : t("sendAgain")}</Button>
          </> : null}
    ...
    {stage === "summary" && checkoutUrl ? <div className="payment-studio-mobile-checkout mt-6 flex flex-wrap items-center gap-4 border border-[var(--border)] bg-[var(--surface-soft)] p-4">
      ...
      <div className="payment-studio-mobile-checkout-copy min-w-[180px] flex-1">
        ...
        <p className="payment-studio-mobile-checkout-description mt-2 text-xs leading-5 text-[var(--text-muted)]">{t("qrHint")}</p>
      </div>
    </div> : null}
    {shareUrl && <div className="payment-studio-share-result mt-6 border border-[var(--border)] bg-[var(--surface)] p-4">
      <p className="payment-studio-share-title flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.14em] text-[var(--positive)]"><CheckCircle2 size={13} />{t("checkoutReady")}</p>
      <label className="payment-studio-share-link-field mt-3 block"><Label className="payment-studio-share-link-label">Payment link</Label><input aria-label={t("paymentLink")} readOnly value={shareUrl} onFocus={e => e.target.select()} className="payment-studio-share-link-input payment-input mt-2" /></label>
  ...
  return <div className="payment-studio-root mx-auto max-w-[1240px]">
    <div className="payment-studio-header mb-8 flex flex-wrap items-end justify-between gap-4 border-b border-[var(--border)] pb-6">
      <div className="payment-studio-heading"><Label className="payment-studio-eyebrow text-[var(--action)]">{checkout ? t("checkoutTitle") : t("workspaceEyebrow")}</Label>
        <h1 className="payment-studio-title mt-3 text-3xl font-semibold tracking-[-0.02em]">{checkout ? t("aPaymentForYou") : initialMode === "request" ? t("tabRequest") : t("workspaceTitle")}</h1>
        <p className="payment-studio-description mt-2 text-sm text-[var(--text-muted)]">{checkout ? t("checkoutSubtitle") : t("workspaceSubtitle")}</p></div>
    ...
    <div className="payment-studio-layout grid items-start gap-6 xl:grid-cols-[1.15fr_0.85fr]">
      <Card className="payment-studio-form-card overflow-hidden">
        {checkout ? form : <AnimatedTabs tabs={[{ id: "pay", label: t("tabSend") }, { id: "request", label: t("tabRequest") }]} value={mode} disabled={busy || stage !== "editing"} onChange={next => { setMode(next); setError(""); setFieldErrors({}); }}>{form}</AnimatedTabs>}
      </Card>
      <div className="payment-studio-aside space-y-5 xl:sticky xl:top-20">
        ...
        <Panel className="payment-studio-preview-panel" title={stage === "success" ? t("paymentReceipt") : t("paymentPreview")} meta={`${ARC.name.toUpperCase()} · USDC`} bodyClassName="p-0">
          ...
          <div className="payment-studio-preview-details px-4 py-4">
            <ReceiptRow label={t("recipient")} value={current.to || t("addRecipient")} />
            <Divider className="payment-studio-preview-divider my-3" />
            <ReceiptRow label="For" value={current.memo || t("yourPayment")} />
            <Divider className="payment-studio-preview-divider my-3" />
            <ReceiptRow label={t("reference")} value={current.reference || t("optional")} />
          </div>
          <div className="payment-studio-preview-totals border-t border-[var(--border)] px-4 py-3">
            <TotalRow label={t("networkFee")} value={totals?.fee ? `${totals.fee} USDC` : t("atReview")} muted />
            {totals?.total ? <><Divider className="payment-studio-preview-total-divider my-2.5" /><TotalRow label={t("totalFromWallet")} value={`${totals.total} USDC`} strong /></> : null}
          </div>
        </Panel>
...
/**
 * Lives outside the component, so it takes the translator rather than
 * reaching for one. The final branch returns the chain's own message, which
 * no catalogue can cover — the two cases above are the ones worth naming.
 */
function friendlyError(e: unknown, t: (key: string) => string) {
  const message = e instanceof Error ? e.message : String(e);
  if (/rejected|denied/i.test(message)) return t("declined");
  if (/insufficient/i.test(message)) return t("insufficient");
  return message.split("\n")[0]?.slice(0, 220) || t("genericError");
}
