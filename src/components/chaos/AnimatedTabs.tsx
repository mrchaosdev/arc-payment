...
export function AnimatedTabs<T extends string>({ tabs, value, onChange, disabled = false, children }: {
  ...
  return (
    <>
      ...
      <div role="tablist" aria-label="Payment action" className="relative flex border-b border-[var(--border)]">
        <span aria-hidden className="absolute bottom-0 left-0 h-px bg-[var(--action)] transition-transform duration-200 motion-reduce:transition-none"
        ...
        {tabs.map((tab, i) => (
          <button key={tab.id} ref={el => { buttons.current[i] = el; }} type="button" role="tab"
            ...
            className={cn("relative h-12 flex-1 font-mono text-[11px] uppercase tracking-[0.14em] transition-colors disabled:opacity-45", value === tab.id ? "text-[var(--action)]" : "text-[var(--text-muted)] hover:text-[var(--text-primary)]")}>
            {tab.label}
          </button>
        ))}
      </div>
